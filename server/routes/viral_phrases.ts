...
router.post('/upload-image', authMiddleware, async (req: any, res) => {
...
router.post('/schedule', authMiddleware, async (req: any, res) => {
  ...
  const userId = req.user.id;
  if (!imageUrl || !scheduledAt) return res.status(400).json({ error: 'imageUrl e scheduledAt são obrigatórios' });
  try {
    const supabase = getSupabase();
...
router.get('/scheduled', authMiddleware, async (req: any, res) => {
  try {
    const supabase = getSupabase();
    const { data, error } = await supabase
      .from('viral_scheduled_posts')
...
router.delete('/scheduled/:id', authMiddleware, async (req: any, res) => {
  try {
    const supabase = getSupabase();
