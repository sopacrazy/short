...
export async function generateImage(
  ...
  if (!apiKey)
    throw new Error(
      "Configuração ausente: Replicate API Token não encontrado para este usuário.",
    );
  ...
  if (typeof output === "string") {
    ...
    throw new Error(
      `Formato de resposta inesperado do Replicate: ${JSON.stringify(output)}`,
    );
  }
...
const STYLE_HINTS: Record<string, string> = {
  ...
  wildlife:
    "wildlife documentary photography, BBC Earth / National Geographic style, " +
  ...
  historical:
    "historical documentary photography, period-accurate reconstruction, " +
  ...
  documentary:
    "editorial documentary photography, National Geographic / BBC editorial style, " +
  ...
  cinematic:
    "shot on ARRI Alexa, RAW photo, photojournalism, natural light, " +
    "shallow depth of field, film grain, ultra-sharp, hyperrealistic",

  // Cenas noturnas urbanas, mistério, tecnologia noir
  cyberpunk:
    "RAW photo, neon-lit urban night scene, wet streets reflection, " +
  ...
  illustration:
    "vibrant 2D digital illustration, bold colors, modern flat art, clean lines",

  // Fotografia de retrato / lifestyle
  realistic:
    "RAW photo, shot on Canon EOS R5 85mm f/1.4, natural lighting, " +
  ...
  underwater:
    "underwater photography, Sony A7IV with underwater housing, " +
  ...
  space:
    "NASA-quality astrophotography, scientifically accurate celestial bodies, " +
    "deep space photography, no fantasy nebula coloring, hyperrealistic",

  // Medicina / corpo humano / saúde
  medical:
    "medical editorial photography, clinical lighting, " +
