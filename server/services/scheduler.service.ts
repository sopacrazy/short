...
async function runScheduledPublications() {
  try {
    ...
    for (const post of posts) await publishScheduledPost(post);
    for (const post of viralPosts) await publishViralScheduledPost(post);
    for (const reel of reels) await publishScheduledReel(reel);
  } catch (err) {
    console.error('[Scheduler] Erro ao verificar fila:', err);
  }
}

async function getViralScheduledPostsDue(): Promise<any[]> {
...
async function publishViralScheduledPost(post: any) {
  try {
...
async function getScheduledReelsDue(): Promise<any[]> {
  const { data } = await getSupabase()
...
async function publishScheduledReel(reel: any) {
  try {
...
async function publishScheduledPost(post: any) {
  try {
    if (!post.composed_image_path) throw new Error('Post sem imagem composta salva');

    const imageUrl = await getComposedImagePublicUrl(post.composed_image_path);
    const hashtagStr = (post.hashtags as string[]).map(h => `#${h}`).join(' ');
