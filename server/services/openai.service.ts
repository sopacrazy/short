...
export async function generateViralPhrase(query: string, apiKey?: string | null, category?: string) {
  if (!apiKey) throw new Error('Configuração ausente: OpenAI API Key não encontrada para este usuário.');
  const client = new OpenAI({ apiKey });
...
export async function translateTexts(texts: string[], targetLang = 'Português (Brasil)', apiKey?: string | null): Promise<string[]> {
