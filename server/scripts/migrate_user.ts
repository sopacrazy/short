...
async function main() {
  ...
  if (projectError) console.error('Erro ao migrar projetos:', projectError.message);
  else console.log('Projetos migrados.');
  ...
  if (folderError) console.error('Erro ao migrar pastas:', folderError.message);
  else console.log('Pastas migradas.');

  // 3. Cadastrar Chaves de IA
  console.log('Cadastrando chaves de IA iniciais...');
  const cleanKey = (k?: string) => k?.split(/\s|#/)[0] || null;
  ...
  if (settingsError) console.error('Erro ao salvar chaves:', settingsError.message);
  else console.log('Chaves cadastradas com sucesso.');
