...
export async function generateCuriositySuggestions(query, count = 5, apiKey) {
    if (!apiKey)
...
export async function generateAdvancedPostImage(titleWhite, titleYellow, description, imagePrompt, apiKey) {
    if (!apiKey)
        throw new Error('Configuração ausente: OpenAI API Key não encontrada para este usuário.');
    const client = new OpenAI({ apiKey });
...
export async function generateCuriosityPost(curiosity, apiKey, language = 'pt') {
    if (!apiKey)
        throw new Error('Configuração ausente: OpenAI API Key não encontrada para este usuário.');
    const client = new OpenAI({ apiKey });
