...
router.get('/', authMiddleware, async (req, res) => {
    ...
    for (const s of scenes ?? []) {
        if (!thumbnailMap[s.project_id])
            thumbnailMap[s.project_id] = s.image_url;
    }
...
router.post('/:id/endcard', authMiddleware, async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
...
router.post('/:id/soundtrack', authMiddleware, async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
    const { soundtrack_url, music_volume } = req.body;
