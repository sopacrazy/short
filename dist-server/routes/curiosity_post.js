...
router.post('/advanced-image', authMiddleware, async (req, res) => {
    ...
    try {
        const keys = await getUserAIKeys(userId);
        const dataUrl = await generateAdvancedPostImage(title_white, title_yellow, description, image_prompt, keys.openai_key);
        ...
        if (!publicData?.publicUrl)
            throw new Error('Não foi possível gerar URL para a imagem');
...
router.post('/upload-composed', authMiddleware, async (req, res) => {
