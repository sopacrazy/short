...
export const Caption = ({ text, durationInFrames }) => {
    ...
    const opacity = interpolate(frame, [0, 10, durationInFrames - 8, durationInFrames], [0, 1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
    const words = text.split(' ');
    const activeIndex = Math.min(Math.floor((frame / Math.max(durationInFrames, 1)) * words.length), words.length - 1);
