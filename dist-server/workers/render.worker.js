...
async function getAudioDuration(audioUrl, ffmpegBin) {
    try {
...
export async function renderVideo(projectId, onProgress) {
    ...
    if (!scenesRes.data?.length)
        throw new Error('Cenas não encontradas — gere o roteiro primeiro');
    ...
    let audioDuration = script.duration_seconds;
    if (audioUrl) {
    ...
    const totalGptDuration = scenes.reduce((sum, s) => sum + s.duration_seconds, 0);
    ...
    const scriptLines = buildCaptionLines(`${script.hook} ${script.body} ${script.cta}`, audioDuration);
    // 2. Atualiza status
    await supabase
        .from('projects')
    ...
    if (!browserExecutable) {
        throw new Error('Chrome não encontrado. Instale o Google Chrome ou defina CHROME_EXECUTABLE no .env');
    }
    ...
    try {
        ...
        console.warn('[Render] Faststart falhou, copiando original:', e.message);
    ...
    if (uploadError)
        throw new Error(`Upload falhou após ${MAX_UPLOAD_ATTEMPTS} tentativas: ${uploadError.message}`);
...
function buildCaptionLines(text, durationSeconds) {
